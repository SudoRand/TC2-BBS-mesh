...
class TestConnectFour(unittest.TestCase):
    ...
    def test_continue_game_flow(self, mock_driver_send):
        ...
        with patch('utils.get_node_id_from_num', side_effect=lambda num, iface: {p1_num: p1_id, p2_num: p2_id}.get(int(num))), \
            ...
            # 1. Player goes to the connect_four menu
            ...
            handle_connect_four_steps(p1_num, str(game_id), state_p1['step'], state_p1, mock_interface)
    ...
    def test_list_current_games_displays_short_name(self, mock_driver_send):
        ...
        with patch('utils.get_node_id_from_num', return_value=p1_id), \
            ...
            handle_connect_four_command('some_other_player', mock_interface)
    ...
    @patch('modules.Games.game_logic_driver.send_message')
    def test_remote_game_flow(self, mock_driver_send, mock_c4_send):
        mock_interface = MagicMock()
        ...
        with patch('utils.get_node_id_from_num', side_effect=lambda num, iface: {p1_num: p1_id, p2_num: p2_id}.get(int(num))), \
            ...
            handle_connect_four_steps(p1_num, "n", 1, {}, mock_interface)
            self.assertEqual(mock_c4_send.call_count, 3)
            ...
            # P2 joins the game
            state_p2 = {'command': 'CONNECT_FOUR', 'step': 1}
            handle_connect_four_steps(p2_num, str(game_id), 1, state_p2, mock_interface)
    ...
    @patch('modules.Games.game_logic_driver.send_message')
    @patch('modules.Games.connect_four.send_message')
    def test_pause_and_resume_game(self, mock_c4_send, mock_driver_send, mock_help):
        mock_interface = MagicMock()
        p1_num = 111; p1_id = '!p1'; p1_sn = 'P1'
        mock_interface.nodes = { p1_id: {'num': p1_num, 'user': {'shortName': p1_sn}} }
        ...
        with patch('utils.get_node_id_from_num', return_value=p1_id), \
            ...
            handle_connect_four_steps(p1_num, "n", 1, {}, mock_interface)
            state_p1 = get_user_state(p1_num)
            handle_connect_four_steps(p1_num, "1", state_p1['step'], state_p1, mock_interface)
            game_id = get_open_games('connect_four')[0][0]

            # P1 pauses the game
            state_p1_ingame = {'command': 'CONNECT_FOUR', 'step': 12, 'game_id': game_id}
            handle_connect_four_steps(p1_num, "x", 12, state_p1_ingame, mock_interface)
    ...
    def test_stats_menu_flow(self, MockGameLogicDriver):
        ...
        handle_connect_four_steps(sender_id, 'm', 2, state_step_2, interface)
