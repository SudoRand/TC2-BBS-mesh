        ...
        try:
            if system_config['interface_type'] == 'serial':
                if system_config['port']:
                    interface = meshtastic.serial_interface.SerialInterface(system_config['port'])
                else:
                    ports = list(serial.tools.list_ports.comports())
                    if len(ports) == 1:
                        interface = meshtastic.serial_interface.SerialInterface(ports[0].device)
                    elif len(ports) > 1:
